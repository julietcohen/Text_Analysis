---
title: "pdf_text_analysis topic 3"
author: "Juliet"
date: "4/13/2022"
...
# work with the joining to categorize neutral words
```
...
sent_scores <- sent_words %>%
  ...
  offset_score = (positive - negative) - offset) %>% #how does this page's sentiment compare to that of the average page? teh average sentiment score is -3 
  arrange(desc(raw_score))
sent_scores # sorted by offset_score so the highest offset score is the first row, it was the most pos with score of 9
```
...
ggplot(sent_scores, aes(x = page)) +
  ...
  labs(title = 'Sentiment analysis: The Overstory',
       y = 'Sentiment score')
# interpretation:
# blue = offset calc result
# red = total score
# the red is the starting number and then subtract -3 to get the score
# we offset to account for the overall trend of the book, which is slightly negative in this case
```
...
nrc_fear <- get_sentiments("nrc") %>% 
  filter(sentiment == "fear") # assigns diff emotional words to the words in the text
...
setwd(here("/Users/juliet/Documents/MEDS/Text_Analysis/Text_Analysis/data/nexis_dat"))
...
# this file is an example because it already has files within it 

dat <- lnt_read(my_files) #Object of class 'LNT output'

# 3 diff tibbles
meta_df <- dat@meta # has metadata
articles_df <- dat@articles # has articles
paragraphs_df <- dat@paragraphs # has paragraphs
...
sent <- sentiment(mytext) # neutral = 0.0000, scale -1 to 1, 0.06 = slightly positive, etc.
# some articles mihgt have been repeated due to 2 headlines, like a subtitle, which might explain the excessive total number of rows
...
```{r}
# PSEUDO CODE
# try to reconstruct the plot from paper, average sentiment score per day
# group data by day, summarize
ggplot(data = data, aes( x= date, y = mean_sentiment))
```
...
3.  Use your search term along with appropriate filters to obtain and download a batch of at least 100 (more or less, just enough to make an interesting plot, you can only get more than 100 if you execute multiple downloads) full text search results (.docx).  

4.  Read your Nexis article document into RStudio.

5.  This time use the full text of the articles for the analysis.  First clean any artifacts of the data collection process (hint: this type of thing should be removed: "Apr 04, 2022(introduction to how to clean text data: Biofuels Digest: http://www.biofuelsdigest.com/ Delivered by Newstex"))
...
He will update the assignment, check for changes

y axis scale = 0-100, total number of emotion words as a percentage 
x = time (day)
geom = line, many lines, one for each emotion
