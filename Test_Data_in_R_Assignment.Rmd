...
query_results <- fromJSON("http://api.nytimes.com/svc/search/v2/articlesearch.json?q=wildlife&api-key=IpTnsNntENJKZIG3hUdas1S5PWeEt1ko", flatten = TRUE) #the string following "key=" is your API key 
...
Create a query for the word "wildlife" spanning from 1 year ago to today (April 9th, 2022) 
...
clean_tokens <- str_replace_all(tokenized$word,"pets","pet") # stem the word "furry" to "fur", I expect to see "furry" often in these articles
#clean_tokens <- str_replace_all(tokenized$word,"pet[a-z,A-Z]*","pet")

clean_tokens <- str_replace_all(tokenized$word,"cats","cat") # stem the word "", which I expect to see often in these articles

clean_tokens <- str_replace_all(tokenized$word,"sleep.+","sleep") # stem the words "sleepy", "sleepiest" etc. to "sleep", I expect to see iterations of the word "sleep" often in these articles
...
The stmmed word "cat" appears the most by a large margin. The second most common word is "people". 
